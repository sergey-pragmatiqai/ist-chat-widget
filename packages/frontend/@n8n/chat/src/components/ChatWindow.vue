...
import WelcomeMessage from '@n8n/chat/components/WelcomeMessage.vue';
...
const isWelcomeMessageEnabled = computed(() => {
	return options.welcomeMessage?.enabled !== false && options.welcomeMessage?.text;
});
...
function onWelcomeMessageClick() {
	toggle();
}
...
onMounted(() => {
	...
	// Show welcome message after a delay if enabled and user hasn't interacted
	if (isWelcomeMessageEnabled.value) {
		setTimeout(() => {
...
<template>
	<div class="chat-window-wrapper">
		...
		<WelcomeMessage
			:show="showWelcomeMessage && !isOpen && !hasInteracted"
			@close="hideWelcomeMessage"
			@click="onWelcomeMessageClick"
		/>
